...
Usage:
  ...
  ./run_test.sh count baseline|cross|exhaustive-ike|exhaustive-esp|exhaustive
  ./run_test.sh check baseline|cross|exhaustive-ike|exhaustive-esp|exhaustive
  ./run_test.sh cleanup
...
Defaults:
  ...
  exhaustive-ike  START=1 LIMIT=100
  exhaustive-esp  START=1 LIMIT=100
...
case "$COMMAND" in
    ...
    exhaustive-ike)
        [[ $# -le 2 ]] || fatal "exhaustive-ike syntax: exhaustive-ike [START] [LIMIT]"
        START="${1:-1}"
        LIMIT="${2:-100}"
        is_positive_integer "$START" || fatal "START must be an integer >= 1"
        is_positive_integer "$LIMIT" || fatal "LIMIT must be an integer >= 1"
        ...
        ensure_app
        check_psk "$CONFIG"
        printf '[INFO] config=%s mode=exhaustive-ike start=%s limit=%s\n' "$CONFIG" "$START" "$LIMIT"
        run_privileged "$APP" --config "$CONFIG" --mode exhaustive-ike --start "$START" --limit "$LIMIT"
        ;;

    exhaustive-esp)
        [[ $# -le 2 ]] || fatal "exhaustive-esp syntax: exhaustive-esp [START] [LIMIT]"
        START="${1:-1}"
        LIMIT="${2:-100}"
        is_positive_integer "$START" || fatal "START must be an integer >= 1"
        is_positive_integer "$LIMIT" || fatal "LIMIT must be an integer >= 1"
        ...
        ensure_app
        check_psk "$CONFIG"
        printf '[INFO] config=%s mode=exhaustive-esp start=%s limit=%s\n' "$CONFIG" "$START" "$LIMIT"
        run_privileged "$APP" --config "$CONFIG" --mode exhaustive-esp --start "$START" --limit "$LIMIT"
        ;;
    ...
    count)
        [[ $# -eq 1 ]] || fatal "count syntax: count baseline|cross|exhaustive-ike|exhaustive-esp|exhaustive"
        MODE="$1"
        case "$MODE" in baseline|cross|exhaustive-ike|exhaustive-esp|exhaustive) ;; *) fatal "unsupported count mode: $MODE" ;; esac
        CONFIG="$(select_config)"
        ensure_app
    ...
    check)
        [[ $# -eq 1 ]] || fatal "check syntax: check baseline|cross|exhaustive-ike|exhaustive-esp|exhaustive"
        MODE="$1"
        case "$MODE" in baseline|cross|exhaustive-ike|exhaustive-esp|exhaustive) ;; *) fatal "unsupported check mode: $MODE" ;; esac
        CONFIG="$(select_config)"
        ensure_app
