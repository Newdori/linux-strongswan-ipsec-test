...
- VICI: swanctl compiled default, or target-specific `/run`/`/var/run` URI
- IKEv2 + PSK
- ESP transport mode
...
Application
  -> swanctl
  -> VICI
  -> charon (started/checked through the ipsec command)
  -> kernel-netlink
  -> Linux XFRM
  -> ESP
```
...
## PetaLinux service-control port

This tree does not require `systemctl` or `charon-systemd`.  Runtime checks use
`ipsec start` and confirm readiness with `swanctl --stats`.  The target rootfs
must contain the strongSwan `swanctl` command and the VICI plugin.
...
After changing the C sources, rebuild `ipsec_app` with the PetaLinux AArch64
SDK.  Updating `run_test.sh` alone is not sufficient because command checking
and daemon startup are implemented in the compiled application.
