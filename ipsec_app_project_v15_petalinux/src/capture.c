...
static void parse_capture_stats(const char *path, capture_result_t *result)
{
    ...
    while (fgets(line, sizeof(line), fp)) {
        int value = 0;
        if ((strstr(line, " packet captured") || strstr(line, " packets captured")) &&
            sscanf(line, "%d", &value) == 1) {
            result->packets_captured = value;
            got_captured = true;
        } else if ((strstr(line, " packet received by filter") ||
                    strstr(line, " packets received by filter")) &&
                   sscanf(line, "%d", &value) == 1) {
            result->packets_received_by_filter = value;
            got_received = true;
        } else if ((strstr(line, " packet dropped by kernel") ||
                    strstr(line, " packets dropped by kernel")) &&
                   sscanf(line, "%d", &value) == 1) {
