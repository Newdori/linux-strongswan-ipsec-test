...
static void save_runtime_diagnostics(const app_config_t *cfg, const char *result_dir)
{
    ...
    /* PC-A previously used namespace-specific VICI sockets.  Record any
     * charon*.vici sockets under /run so a stale custom socket is obvious. */
    char sockets[8192];
    size_t used = 0;
    sockets[0] = '\0';
    DIR *dir = opendir("/run");
        ...
        while ((ent = readdir(dir)) != NULL) {
            if (strstr(ent->d_name, "charon") && strstr(ent->d_name, ".vici")) {
                int n = snprintf(sockets + used, sizeof(sockets) - used,
                                 "/run/%s\n", ent->d_name);
                if (n < 0 || (size_t)n >= sizeof(sockets) - used) break;
                used += (size_t)n;
            }
        }
        closedir(dir);
    ...
    if (used == 0) {
        snprintf(sockets, sizeof(sockets), "<no charon*.vici entries found under /run>\n");
    }
    save_output(result_dir, "vici_sockets.txt", sockets);

    char target[1024];
    snprintf(target, sizeof(target),
             "configured_vici_uri=%s\nexpected_host_vici_uri=unix:///run/charon.vici\n",
...
{
    char *out = NULL;
    char *active[] = {"systemctl", "is-active", "--quiet", (char *)cfg->service_name, NULL};
    int rc = process_run(active, &out);
    free(out);

    if (rc != 0) {
        char *enabled[] = {"systemctl", "is-enabled", (char *)cfg->service_name, NULL};
        out = NULL;
        int enabled_rc = process_run(enabled, &out);
        save_output(result_dir, "service_enabled_state.txt", out ? out : "");
        if (out && strstr(out, "masked")) {
            log_error("%s is masked. This is common after the previous namespace test. "
                      "Run: sudo systemctl unmask %s",
                      cfg->service_name, cfg->service_name);
            free(out);
            return -1;
        }
        (void)enabled_rc;
        ...
        log_info("starting %s", cfg->service_name);
        char *start[] = {"systemctl", "start", (char *)cfg->service_name, NULL};
        rc = process_run(start, &out);
        save_output(result_dir, "service_start.txt", out);
        if (rc != 0) {
            log_error("failed to start %s: %s", cfg->service_name, out ? out : "");
            free(out);
            return -1;
        }
        free(out);
    ...
    save_output(result_dir, "swanctl_stats.txt", out);
    if (rc != 0) {
        log_error("VICI/charon is not ready: %s", out ? out : "");
        free(out);
        return -1;
    }
    free(out);
    ...
    if (cfg->vici_uri[0] && strcmp(cfg->vici_uri, "unix:///run/charon.vici") == 0) {
        if (access("/run/charon.vici", F_OK) != 0) {
            log_error("configured host VICI socket /run/charon.vici does not exist; "
                      "PC-A may still have the host strongSwan service stopped/masked from the old namespace test");
            return -1;
        }
    }

    log_pass("strongSwan service and VICI are ready");
    return 0;
}
...
int strongswan_load_configuration(const app_config_t *cfg, const char *result_dir)
{
...

    {
        ...
        if (stat(SWANCTL_RUNTIME_DIR, &swanctl_dir_st) != 0 ||
            !S_ISDIR(swanctl_dir_st.st_mode)) {
            log_error("%s does not exist or is not a directory", SWANCTL_RUNTIME_DIR);
            goto cleanup;
        }
    }
